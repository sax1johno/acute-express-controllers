 ...
 * 
 * Controllers are given access to services that are injected into the architecture.
 **/
 ...
 * directory.  Basedir is the base directory for the package that contains 
 * the controllers
 * and dirname is the directory that the controllers are contained in.
 * @param basedir the base directory to find the controllers.
 * @param dirname the directory name of the controllers directory
 * @param injector the service injection container
 * @param fn a callback that is called when the controllers are finished 
 *        being loaded.
 **/
var loadFromFS = function(basedir, dirname, injector, fn) {
    ...
    acuteUtils.walkFs(path.join(basedir, dirname), function(err, files) {
        if (err) {
          ...
          async.each(files, function(file, cb) {
            ...
            var controller = require(file)(app.Router(), injector);
 ...
 * @param injector a dependency injection container that allows controllers
 *        to access injected services.
 * @param fn a callback that's called when the controllers are loaded.
 * 
 **/
var load = function(injector, fn) {
    loadFromFS(config.controller_basedir, config.controller_dirname, injector, fn);
};
